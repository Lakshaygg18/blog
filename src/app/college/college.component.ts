...
univdata:any [] = [];
public isCloseOnSelectEnabled: boolean = true;
@Input() selectedValue :any;
@Output() onChangeData : EventEmitter<any>= new EventEmitter<any>();
@ViewChild('select') select!: NgSelectComponent;

  constructor(private downloadService:DropdownService){
    // this.downloadService.getData().subscribe((data)=>{
    //    this.users = Array(data);

    //   // this.users = this.users.users
    //   // this.univdata = Object.keys(this.users).map(key => ({type: key, value: this.users[key]}));
  ...
  ngOnInit():void{
    // console.log(typeof(this.univdata))
  }
  
  onSelectedValueChange() {
    this.selectedData = this.users.find(item => item.id === this.selectedValue);
    // console.log("college selected value",this.selectedData)
       this.onChangeData.emit({ obj: this.selectedData });
       this.isCloseOnSelectEnabled = true;
       this.select.close();
   }
